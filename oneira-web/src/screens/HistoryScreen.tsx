...
const Chip: React.FC<{ label: string; active?: boolean }> = ({ label, active }) => (
    <button className={`flex h-9 items-center justify-center rounded-full px-5 text-sm font-medium transition-all ${active
...
const NavBtn: React.FC<{ icon: React.ReactNode; active?: boolean; onClick?: () => void }> = ({ icon, active, onClick }) => (
    <button
        onClick={onClick}
        className={`flex items-center justify-center h-12 w-12 rounded-full transition-all ${active
                ? 'bg-primary text-white shadow-[0_0_15px_rgba(255,0,132,0.4)]'
