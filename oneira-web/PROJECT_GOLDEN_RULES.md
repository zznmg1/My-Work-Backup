...
**[핵심 원칙] AI는 모든 작업(코드 수정, 빌드, 환경 설정 변경 등) 시 반드시 [현재 시간]과 [커밋/버전 정보]를 기록해야 하며, 사용자가 언제든 특정 시점으로 롤백을 선언할 수 있도록 보장해야 한다.** 이 룰은 본 프로젝트뿐만 아니라 향후 진행될 모든 관련 프로젝트에 공통 적용된다.
...
*   **실시간 기록 의무**: AI는 모든 의미 있는 작업 단위(예: 고유 기능 구현, 버그 수정)가 끝날 때마다 커밋을 수행하고, 해당 시점의 **시간**을 기록해야 합니다.
...
*   **빌드 감사 로그 (Build Audit Log)**: 모든 빌드 성공 시 `BUILD_HISTORY.log` 파일에 [시간, 커밋ID, 파일명, 주요 변경사항]을 누락 없이 기록합니다.
